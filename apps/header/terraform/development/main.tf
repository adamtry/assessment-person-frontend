...
resource "aws_s3_bucket" "frontend-bucket-development" {
  bucket = "lbh-single-view-header-frontend-development.hackney.gov.uk"
  acl    = "private"
  versioning {
    enabled = true
  }
...
module "cloudfront-development" {
  ...
  orginin_access_identity_desc = "Person Viewer header frontend cloudfront identity"
  cname_aliases = []
...
resource "aws_ssm_parameter" "cdn" {
  name  = "/single-view/development/header-app-url"
  type  = "String"
